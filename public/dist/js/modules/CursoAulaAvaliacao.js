const CursoAulaAvaliacao = (function () {
  const _formIdPrincipal = "#form-cursoAulaAvaliacao-principal";
  const _formId = "#cursoAulaAvaliacaoForm";
  const _tableId = "#cursoAulaAvaliacaoTable";
  const _tableTdClass = "cursoAulaAvaliacaoTableTd";
  const _modalId = "#cursoAulaAvaliacaoModal";
  ...
  function __createTable() {
    $(_tableId).dataTable({
      ...
      bAutoWidth: false,
      ajax: {
        url: "/admin/cursos-aulas-avaliacoes/data",
        type: "GET",
  ...
  function __loadModulosFiltro(id) {
    ...
    window
      .axios({
        url: `/admin/cursos-aulas-avaliacoes/curso/${id}/modulos`,
        method: "GET",
      })
      .then(function (resp) {
  ...
  function __add() {
    ...
    $(".modal-content", _modalId).load(
      "/admin/cursos-aulas-avaliacoes/create",
  ...
  function __update(id) {
    ...
    $(".modal-content", _modalId).load(
      "/admin/cursos-aulas-avaliacoes/" + id + "/edit",
      function (responseText, textStatus, jqXHR) {
        if (textStatus === "success") {
          if (Validator.IsJsonString(responseText)) {
            var json = JSON.parse(responseText);
  ...
  function __save() {
    ...
    var url = id
      ? "/admin/cursos-aulas-avaliacoes/" + id
      : "/admin/cursos-aulas-avaliacoes";
  ...
  function __delete() {
    Notify.confirm(
      ...
      function () {
        $.ajax({
          type: "DELETE",
          url: "/admin/cursos-aulas-avaliacoes/" + $("#id", _formId).val(),
          dataType: "json",
          timeout: 120000,
          success: function (json) {
