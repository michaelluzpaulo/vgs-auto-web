const Transacao = (function () {
  const _formIdPrincipal = "#form-transacao-principal";
  const _formId = "#transacaoForm";
  const _tableId = "#transacaoTable";
  const _tableTdClass = "transacaoTableTd";
  const _modalId = "#transacaoModal";
  ...
  function __add() {
    $.loadmask();
    ModalFactory.create("transacaoModal", "xl");

    $(".modal-content", _modalId).load(
      "/admin/transacao/create",
  ...
  function __save() {
    ...
    var url = id ? "/admin/transacao/" + id : "/admin/transacao";
  ...
  function __delete() {
    Notify.confirm(
      ...
      function () {
        $.ajax({
          type: "DELETE",
          url: "/admin/transacao/" + $("#id", _formId).val(),
          dataType: "json",
          timeout: 120000,
          success: function (json) {
