const CursoAulaDuvida = (function () {
  const _formIdPrincipal = "#form-cursoAulaDuvida-principal";
  const _formId = "#cursoAulaDuvidaForm";
  const _tableId = "#cursoAulaDuvidaTable";
  const _tableTdClass = "cursoAulaDuvidaTableTd";
  const _modalId = "#cursoAulaDuvidaModal";
  ...
  function __createTable() {
    $(_tableId).dataTable({
      ...
      bAutoWidth: false,
      ajax: {
        url: "/admin/cursos-aulas-duvidas/data",
        type: "GET",
  ...
  function __add() {
    ...
    $(".modal-content", _modalId).load(
      "/admin/cursos-aulas-duvidas/create",
  ...
  function __update(id) {
    $.loadmask();
    ModalFactory.create("cursoAulaDuvidaModal", "xl");

    $(".modal-content", _modalId).load(
      "/admin/cursos-aulas-duvidas/" + id + "/edit",
      function (responseText, textStatus, jqXHR) {
        if (textStatus === "success") {
          if (Validator.IsJsonString(responseText)) {
            var json = JSON.parse(responseText);
  ...
  function __save() {
    ...
    var url = id
      ? "/admin/cursos-aulas-duvidas/" + id
      : "/admin/cursos-aulas-duvidas";
  ...
  function __delete() {
    Notify.confirm(
      ...
      function () {
        $.ajax({
          type: "DELETE",
          url: "/admin/cursos-aulas-duvidas/" + $("#id", _formId).val(),
          dataType: "json",
          timeout: 120000,
          success: function (json) {
