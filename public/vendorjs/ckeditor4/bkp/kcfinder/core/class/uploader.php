...
class uploader
{
  ...
  public function __construct()
  {
    ...
    elseif (!mb_strlen($this->config['cookieDomain']))
    ...
    if (preg_match(
      ...
      $this->config['uploadURL'] = "$protocol://$domain" . (mb_strlen($port) ? ":$port" : "") . "/$path";
    ...
    if (isset($_GET['CKEditorFuncNum'])) {
      ...
      if ($_GET['opener'] == "tinymce") {
        if (!isset($this->config['_tinyMCEPath']) || !mb_strlen($this->config['_tinyMCEPath']))
  ...
  public function upload()
  {
    ...
    if ($config['disabled'] || !$config['access']['files']['upload']) {
      ...
      if (
        ...
        if (substr($udir, 0, mb_strlen($dir)) !== $dir)
      ...
      if (!mb_strlen($message)) {
        ...
        if (
          ...
          if (preg_match('/^([a-z]+)\:\/\/([^\/^\:]+)(\:(\d+))?\/(.+)$/', $url, $patt)) {
            list($unused, $protocol, $domain, $unused, $port, $path) = $patt;
            $base = "$protocol://$domain" . (mb_strlen($port) ? ":$port" : "") . "/";
  ...
  protected function checkFilePath($file)
  {
    ...
    return (substr($rPath, 0, mb_strlen($this->typeDir)) === $this->typeDir);
  }
  ...
  protected function validateExtension($ext, $type)
  {
    ...
    if (!mb_strlen($exts) || substr($exts, 0, 1) == "*")
      return true;
  ...
  protected function imageResize($image, $file = null)
  {
    ...
    if ((!isset($this->config['watermark']['file']) ||
        (!mb_strlen(trim($this->config['watermark']['file'])))
      ) && (
  ...
  protected function makeThumb($file, $overwrite = true)
  {
    ...
    $thumb = substr($file, mb_strlen($this->config['uploadDir']));
